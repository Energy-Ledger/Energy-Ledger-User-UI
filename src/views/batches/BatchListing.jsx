...
import BatchFilter from '../../components/common/BatchFilter';
      ...
      const getBatches = async () => {
        ...
        try {
          if(refreshBatchList){
            setBatchList([]);
            console.log(user);
            let batches = await batchService.getUserBatches(user.wallet_address, user.role, page ,  perPage , search);
            dispatch({
              type: BATCHES,
              payload: { totalBatches: batches.data.pagination.totalCount },
            });

            setBatchList(batches.data.batches)
            setTotalCount(batches.data.pagination.totalCount)
  ...
  const columns = [
    {
      accessor: "action", Header: "QR COde", Cell: ({ row }) => (
        // <div>   QRCode </div>)
        <div>   <QRCode value={`${configService.getBaseUrl()}batch/${row.original.batch_id}`} size={30} /> </div>)
    },
    ...
    //   Cell: ({ row }) => (
    //     <>
    //     {Number(row.original.block_number)|| ""}
    //     </>
    //   ),
    // },
    {
